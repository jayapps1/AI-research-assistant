...
export function HomeDashboard() {
  ...
  if (workspaces.isLoading) return <PageLoading label="Loading dashboard" />;
  if (workspaces.isError) return <ErrorState error={workspaces.error} onRetry={() => workspaces.refetch()} />;
  return (
    <section className="page">
      ...
      <div className="grid cols-3">
        ...
        <Card><div className="stat"><span>Workspace role</span><strong>{workspace?.role ?? 'Member'}</strong></div></Card>
      </div>
      <div className="grid cols-2" style={{ marginTop: 16 }}>
        <Card>
          <h2>Recent projects</h2>
          {pageContent(projects.data).length ? pageContent(projects.data).slice(0, 5).map((project) => (
            <p key={project.id}><Link to={`/projects/${project.id}`}>{project.title}</Link> <Badge>{project.status ?? 'Active'}</Badge></p>
      ...
      <div className="grid cols-3">
        {query.data?.map((workspace) => (
          <Card key={workspace.id}>
            ...
            <p><Badge tone="info">{workspace.type ?? 'Workspace'}</Badge> <Badge>{workspace.role ?? 'Member'}</Badge></p>
            <Button asChild variant="secondary"><Link to={`/workspaces/${workspace.id}/projects`}>View projects</Link></Button>
          </Card>
        ))}
      </div>
    </section>
...
export function ProjectsPage() {
  ...
  const create = useMutation({
    ...
    onSuccess: () => client.invalidateQueries({ queryKey: ['projects', workspace?.id] }),
  });
  return (
    <section className="page">
      <div className="page-header">
  ...
  return (
    <section className="page">
      ...
      <div className="grid cols-3">
        ...
        <Card><div className="stat"><span>Documents</span><strong>{documents.data?.length ?? '...'}</strong></div></Card>
...
function ProjectSections({ projectId }: { projectId: string }) {
  ...
  return <div className="grid cols-3" style={{ marginTop: 16 }}>{sections.map((section) => <Card key={section}><h2>{section}</h2><Button asChild variant="secondary"><Link to={`/${section.toLowerCase().replaceAll(' ', '-').replace('&', 'analysis')}?projectId=${projectId}`}>Open</Link></Button></Card>)}</div>;
}
